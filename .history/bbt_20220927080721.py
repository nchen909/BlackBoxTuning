...
parser.add_argument("--alg", default='CMA', type=str, choices=['CMA','openes','PEPG','DFO_tr','Nelder-Mead','Powell'], help='select alg from CMA, openes, PEPG, DFO_tr, Nelder-Mead, Powell, SLSQP, COBYLA, BFGS etc')
...
class LMForwardAPI:
    def __init__(self, model_name='roberta-large', n_prompt_tokens=50, task_name='SST-2', loss_type='hinge', init_prompt_path=None):
        ...
        # self.linear(z)= ( 500*(50*256) A) *z, just open a space to store A, torch.nn.init.normal_ init A
        if random_proj == 'normal':
            ...
            for p in self.linear.parameters():
                print("self.linear.parameters() p:", p)
                torch.nn.init.normal_(p, mu, std)
            # Fills the input Tensor A with values drawn from the normal distribution (mean,std2) N(mean,std 2).
        self.best_train_perf = 0.0
        self.best_dev_perf = 0.0
        self.best_prompt = None
        self.num_call = 0
        self.is_better_dev = 0
    ...
    def calc_metric(self, logits, target):
        ...
        if self.loss_type == 'hinge':#but论文里说是交叉熵
    ...
    def eval_bb(self,x):#x:500d
        self.num_call += 1
        bsz = len(dev_data['input_ids'])#48
        prompt_embedding=x
        ...
        prompt_embedding = prompt_embedding.reshape(n_prompt_tokens, -1).repeat(bsz, 1, 1)#bsz=devdatalen=48
        #50*48 matrix
        ...
        with torch.no_grad():
            ...
            # print("type(logits),logits[0][0]:",type(logits),logits[0][0])
        ...
        if self.num_call % 10 == 0:
            ...
            if self.num_call % self.eval_every == 0:
                ...
                print("dev_perf ,self.best_dev_perf:",dev_perf ,self.best_dev_perf)
                if dev_perf > self.best_dev_perf:
                    print("dev_perf > self.best_dev_perf,dev_perf ,self.best_dev_perf:",dev_perf ,self.best_dev_perf)
                    ####let us store x (500*20)(to be DFO_tr start point), mention to use result[5] and try DFO_tr 
                    print("tmp_prompt:",tmp_prompt)
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        elif isinstance(prompt_embedding, np.ndarray):  # single query or None, minimize turns to ndarray
            prompt_embedding = torch.tensor(prompt_embedding).type(torch.float32)  # z
            prompt_embedding = prompt_embedding if len(prompt_embedding.shape)==1 else prompt_embedding.reshape(-1)
        ...
        if isinstance(test_data, DataSet):
            ...
            with torch.no_grad():
                if model_name in ['t5-small', 't5-base', 't5-large', 't5-3b']:
                    ...
                    # print("type(logits),logits:",type(logits),logits)
            ...
            if parallel:
                ...
                return loss
    def refresh_is_better_dev(self):
        self.is_better_dev = 0
        return self.is_better_dev
...
# xi=[np.array(500*[1])]
# print(xi[0].size)
# res=minimize(model_forward_api.eval,x0=xi[0], method='Nelder-Mead',callback=print)

if alg.lower()=='cma':
    cma_or_escma='escma'# CMA by import cma or es 
    if cma_or_escma=='cma':
        cma_opts = {
            'seed': seed,
        ...
        while not es.stop():
            solutions = es.ask()
            # print("len(solutions),solutions[0]:",len(solutions),solutions[0])
            ...
            if parallel:
                ...
                # print("len(solutions),solutions[0].size:",len(solutions),solutions[0].size)#20,500
                # print("type(solutions[0]):",type(solutions[0]))
            ...
            result = es.result()#[0]:solution [1]:best f value [2]
        ...
        while model_forward_api.num_call < budget:
            if model_forward_api.num_call % 100 < 100:#80:
        ...
        solver.plot()
        savefig("./cmaplot.png")
elif alg.lower() in ['dfo_tr']:
    dfo_or_cma="dfo"
    # parallel = True # for not evolution strategy, but it needs x to be a list
    def negative_api(x_):#[500,1] to [20]
        start_time = time.time()
        if 1:
            fitness = model_forward_api.eval(x_)
            return fitness
        else:
            fitness_list = np.zeros(popsize)
            for i in range(popsize):
                fitness_list[i] = model_forward_api.eval(x_)
            return fitness_list
    if dfo_or_cma == "dfo":
        options={"maxfev": budget, "init_delta": 5,
                "tol_delta": 1e-10, "tol_f": 1e-10, "tol_norm_g": 1e-5,
                "sample_gen": "auto", "verbosity": 0}
        
        import os
        if os.path.exists('result5.pickle'):
            with open('result5.pickle', 'rb') as file:
                result5 = pickle.load(file)
                res = dfo_tr(negative_api,      # number of model parameters
                result5,
                options,
                )
        else:
            res = dfo_tr(negative_api,      # number of model parameters
                np.array(intrinsic_dim*[0]),
                options,
            )
        print("loss(fitnesses):",res.fun)
    elif dfo_or_cma == "cma":
        x_0=np.array(intrinsic_dim*[0])
        cma_opts = {
            'seed': 8,
            'popsize': 20,
            'maxiter': 100,
        ...
        es = cma.CMAEvolutionStrategy(x_0, 1, inopts=cma_opts)
        res =es.optimize(negative_api).result
        print("loss(fitnesses):",res.fbest)
    ...
    #         )
    solver = es.OpenES(intrinsic_dim,                  # number of model parameters
            seed=seed,
    ...
    while model_forward_api.num_call < budget:
        ...
        for i in range(solver.popsize):
            fitness_list[i] = -model_forward_api.eval(solutions[i])
            # in RL, rewards needs to be maximize, so es.py assumes to ascent
        solver.tell(fitness_list)
        result = solver.result()
        print("#####result[1]:",result[1])
        print("#####fitness_list[0]:",fitness_list[0])
    ...
    #         #    weight_decay=0.01    # weight decay coefficient)
    #         )
    solver = es.PEPG(intrinsic_dim,
        sigma_init=sigma_init,              # initial standard deviation
    #    sigma_alpha=0.5,             # learning rate for standard deviation
        sigma_decay=1,            # anneal standard deviation
    ...
    while model_forward_api.num_call < budget:
        ...
        if result[1] >10:
            break
elif alg.lower() in ['nelder-mead', 'powell', 'slsqp', 'cobyla', 'bfgs','l-bfgs-b']:
    parallel = False
    options={}
    for methods in ['Powell','Nelder-Mead']:#, "maxfev": 100
        options[methods]={"disp": True, "maxfev":budget if parallel else budget // popsize}
    for methods in ['BFGS','SLSQP','L-BFGS-B']:#, "maxiter": 80/200
        options[methods]={"disp": True, "maxiter":50}
    for methods in ['COBYLA']:
        options[methods]={"disp": True}
    ...
    if parallel:
        ...
        print('parallel, repeat train_data')
    ...
    if os.path.exists('result5.pickle'):
        with open('result5.pickle', 'rb') as file:
            result5 = pickle.load(file)
            fitnesses = minimize(model_forward_api.eval_bb,  result5 , callback=print, method=alg, options=options[alg])
    else:
        fitnesses = minimize(model_forward_api.eval_bb,  [50*np.sin(i) for i in range(intrinsic_dim)], callback=print, method=alg, options=options[alg])
    print("loss(fitnesses):",fitnesses)
else:
    start_time = time.time()
    print('please select alg from CMA, openes, PEPG, DFO_tr, Nelder-Mead, Powell, SLSQP, COBYLA, BFGS etc')
end_time = time.time()
