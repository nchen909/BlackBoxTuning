...
if alg.lower()=='cma':
    ...
    while not es.stop():
        ...
        if parallel:
            ...
            print("model_forward_api.eval(prompt_embedding=np.array(500*[0])):",model_forward_api.eval(prompt_embedding=np.array(500*[0.1])))
            ...
            print("model_forward_api.eval(np.array(500*[0.2]),np.array(500*[0.2])):",model_forward_api.eval(prompt_embedding=np.array(500*[0.2]),test_data=np.array(500*[0.2])))
        ...
        # es.disp()
    es.logger.plot()
elif alg.lower() in ['nelder-mead', 'powell', 'slsqp', 'cobyla', 'bfgs']:
    # parallel=True
    options={}
    for methods in ['Powell','Nelder-Mead']:#, "maxfev": 100
        options[methods]={"disp": True, "maxfev":budget if parallel else budget // popsize}
    for methods in ['BFGS','SLSQP']:#, "maxiter": 80/200
        options[methods]={"disp": True, "maxiter":budget if parallel else budget // popsize}
    ...
    start_time = time.time()
    fitnesses = minimize(model_forward_api.eval, intrinsic_dim *[0], method=alg, options=options[alg])
