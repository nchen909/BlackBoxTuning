...
def convert_to_features(example_batch, tokenizer):
    ...
    target_encodings = tokenizer.batch_encode_plus(
        example_batch['target_text'], add_special_tokens=False)
    mask_pos = []
...
class SST2Loader(Loader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            ...
            ######################prompt在这里改就是生成的hard或soft prompt
            ...
            example['input_text'] = '%s . It was %s .' % (
                example['text'], self.tokenizer.mask_token)
            example['target_text'] = self.label2text[example['labels']]
        return example
    ...
    def _load(self, split, seed) -> DataSet:
        ...
        dataset = dataset.map(self.convert_examples,
                              load_from_cache_file=False)
        print('Example in {} set:'.format(split))
        print(dataset[0])
        dataset = dataset.map(partial(
            convert_to_features, tokenizer=self.tokenizer), batched=True, load_from_cache_file=False)
...
class YelpPLoader(Loader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            ...
            prompt = self.tokenizer.decode(
                list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = '%s . %s .' % (
                prompt, example['text'].replace("\\n", " "))
            example['target_text'] = self.label2text[example['labels']]
        else:
    ...
    def _load(self, split, seed) -> DataSet:
        ...
        def convert_to_features_yelp(example_batch, tokenizer):
            input_encodings = tokenizer.batch_encode_plus(
                example_batch['input_text'], max_length=509, truncation=True)
            target_encodings = tokenizer.batch_encode_plus(
                example_batch['target_text'], add_special_tokens=False)
            template = tokenizer.encode(
                f'It was {tokenizer.mask_token}', add_special_tokens=False)
            input_encodings['input_ids'] = [
                ids[:-1] + template + [tokenizer.sep_token_id] for ids in input_encodings['input_ids']]
            input_encodings['attention_mask'] = [am + [1] *
                                                 3 for am in input_encodings['attention_mask']]
            mask_pos = []
            for input_ids in input_encodings['input_ids']:
                mask_pos.append(input_ids.index(tokenizer.mask_token_id))
            encodings = {
        ...
        dataset = dataset.map(partial(
            convert_to_features_yelp, tokenizer=self.tokenizer), batched=True, load_from_cache_file=False)
        # Convert to fastNLP.DataSet
        ds = DataSet()
        for ins in dataset:
            if len(ins["input_ids"]) <= 512:
...
class AGNewsLoader(Loader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            ...
            prompt = self.tokenizer.decode(
                list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = '%s . %s News: %s' % (
                prompt, self.tokenizer.mask_token, example['text'])
            example['target_text'] = self.label2text[example['labels']]
        else:
            example['input_text'] = '%s News: %s' % (
                self.tokenizer.mask_token, example['text'])
            example['target_text'] = self.label2text[example['labels']]
        return example
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            ...
            example['input_text'] = '%s ? %s , %s' % (
                example['text1'], self.tokenizer.mask_token, example['text2'])
            example['target_text'] = self.label2text[example['labels']]
        return example
...
class SNLILoader(Loader):
    def __init__(self, tokenizer=None, n_prompt_tokens=50):
        ...
        if tokenizer is None:
            ...
            self.tokenizer = tokenizer
        self.n_prompt_tokens = n_prompt_tokens
        def change_SNLI_verb(verb_,change_):
            if verb_ == 'No':
                return 'Unless'
            elif verb_ == 'Maybe':
                return 'Fortunately'
                ...
                return 'Regardless'
        self.label2text = {
            0: change_SNLI_verb("Yes"),
            1: change_SNLI_verb("Maybe"),
            2: change_SNLI_verb("No"),
        }
    ...
    def _load(self, split, seed) -> DataSet:
        ...
        dataset = dataset.filter(
            lambda example: example['labels'] in [0, 1, 2])
        dataset = dataset.map(self.convert_examples,
                              load_from_cache_file=False)
        print(dataset[0])
        dataset = dataset.map(partial(
...
class TRECLoader(Loader):
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            ...
            prompt = self.tokenizer.decode(
                list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = '%s . %s question: %s ' % (
                prompt, self.tokenizer.mask_token, example['text'])
            example['target_text'] = self.label2text[example['labels']]
        else:
            example['input_text'] = '%s . Topic: %s' % (
                self.tokenizer.mask_token, example['text'])
            example['target_text'] = self.label2text[example['labels']]
        return example
