...
parser.add_argument("--budget", default=6000, type=int)
...
if alg.lower()=='cma':
    ...
    if cma_or_escma=='cma':
        ...
        # # print(type(es.result))
        # print("loss(fitnesses):",es.result.fbest)#fbest returns the best fitnesses(loss)
        ...
        while not es.stop():
            ...
            if parallel:
                ...
                # print("model_forward_api.eval(prompt_embedding=np.array(500*[0])):",model_forward_api.eval(prompt_embedding=np.array(500*[0])))
                # print("model_forward_api.eval(prompt_embedding=np.array(500*[0.1])):",model_forward_api.eval(prompt_embedding=np.array(500*[0.1])))
                # print("model_forward_api.eval(prompt_embedding=np.array(500*[0.2])):",model_forward_api.eval(prompt_embedding=np.array(500*[0.2])))
        ...
        for i in range(solver.popsize):
            fitness_list_[i] = -model_forward_api.eval(result[5])
        for i in range(solver.popsize):
            fitness_list[i] = -model_forward_api.eval(result[5])
        for i in range(solver.popsize):
            fitness_list[i] = -model_forward_api.eval(result[5])
