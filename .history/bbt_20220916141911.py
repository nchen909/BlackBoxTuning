...
xi=np.array(10 *[0])
xj=[elif]
res=opt.minimize(y,x0=xi,callback=xi.append,constraints=({'type':'ineq','fun':cons}))
...
if alg.lower()=='cma':
    ...
    for methods in ['BFGS','SLSQP','L-BFGS-B']:#, "maxiter": 80/200
        options[methods]={"disp": True, "maxiter":budget if parallel else budget // popsize,"iprint":101}
    ...
    start_time = time.time()
    fitnesses = minimize(model_forward_api.eval, np.array(intrinsic_dim *[0]), method=alg, options=options[alg])
