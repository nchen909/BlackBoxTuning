...
parser.add_argument("--alg", default='CMA', type=str, choices=['CMA','openes','PEPG','DFO_tr','Nelder-Mead','Powell','SLSQP','COBYLA','L-BFGS-B'], help='select alg from CMA, openes, PEPG, DFO_tr, Nelder-Mead, Powell, SLSQP, COBYLA, BFGS etc')
...
if alg.lower()=='cma':
    ...
    def negative_api(x_):#[500,1] to [20]
        ...
        if 1:
            ...
            return fitness_list
    if dfo_or_cma == "dfo":
        options={"maxfev": budget, "init_delta": 2,
    ...
    if os.path.exists('result5.pickle'):
        with open('result5.pickle', 'rb') as file:
            result5 = pickle.load(file)
            fitnesses = minimize(model_forward_api.eval,  result5 , method=alg, options=options[alg])#, callback=print
    else:
        fitnesses = minimize(model_forward_api.eval,  [50*np.sin(i) for i in range(intrinsic_dim)], callback=print, method=alg, options=options[alg])
