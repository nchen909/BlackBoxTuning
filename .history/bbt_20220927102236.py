...
parser.add_argument("--alg", default='CMA', type=str, choices=['CMA','openes','PEPG','DFO_tr','Nelder-Mead','Powell','SLSQP','COBYLA'], help='select alg from CMA, openes, PEPG, DFO_tr, Nelder-Mead, Powell, SLSQP, COBYLA, BFGS etc')
...
if alg.lower()=='cma':
    ...
    if os.path.exists('result5.pickle'):
        with open('result5.pickle', 'rb') as file:
            result5 = pickle.load(file)
            fitnesses = minimize(model_forward_api.eval,  result5 , callback=print, method=alg, options=options[alg])
    else:
