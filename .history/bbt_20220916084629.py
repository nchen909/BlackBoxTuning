...
if alg.lower()=='cma':
    ...
    for methods in ['Powell','Nelder-Mead']:#, "maxfev": 100
        options['methods']={"seed": seed, "disp": True, "maxfev":budget if parallel else budget // popsize}
    for methods in ['BFGS','SLSQP']:#, "maxiter": 80/200
        options['methods']={"seed": seed, "disp": True, "maxiter":budget if parallel else budget // popsize}
    for methods in ['COBYLA']:
        options['methods']={"seed": seed, "disp": True}
    options_Newton={"seed": seed, "disp": True, "maxiter":budget if parallel else budget // popsize}
    options['Powell'].update({"seed": seed, "disp": True, "ftol": 1e-26}) 
    options['Nelder-Mead'] = {"seed": seed, "disp": True, "ftol": 1e-26, "maxfev":budget}#, "maxfev": 100
    options['COBYLA'] ={"seed": seed, "disp": True, "tol": 1e-25}
    options['BFGS'] ={"seed": seed, "disp": True, "gtol": 1e-5, "maxiter": budget}
    options['SLSQP'] ={"seed": seed, "disp": True, "ftol": 1e-26, "maxiter": budget}#, "maxiter": 200
    start_time = time.time()
